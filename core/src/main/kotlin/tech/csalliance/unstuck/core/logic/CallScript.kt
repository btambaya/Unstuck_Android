...
""".trim()
    ...
    fun conversationRule(kind: CallKind): String = when (kind) {
        ...
        CallKind.AFTER_BLOCK ->
            "Listen. Tick it off (complete_task, or complete_occurrence for a recurring one) ONLY when they clearly say they finished it — \"done\", \"finished it\", \"all done\". If they only say how it went (\"it went well\", \"sort of\") or you didn't catch it, ask one short question first — \"Want me to mark it done?\" — and act only on a yes. Not now → skip_occurrence / set_task_later, and needs another go → schedule_task or block_time, only when they say so. Never change anything they didn't ask for. Act ONLY through tools."
    }
}
