...
// and filters the voice tool schemas to `callTools`, adding the call-level
// `snooze_call`.
//
...
object CallScript {
    /** Tools live during a call. `snooze_call` is CALL-LEVEL: the voice
     *  service handles it locally (outcome `snoozed` + snoozeMinutes, then
     *  hangs up) — it never reaches the app's tool executor. */
    val CALL_TOOLS: List<String> = listOf("complete_task", "add_capture", "schedule_task", "start_focus", "update_call", "snooze_call")

    fun callTools(): List<String> = CALL_TOOLS
    ...
    fun opening(
        ...
        val greeting = preferredName?.trim()?.takeIf { it.isNotEmpty() }?.let { "Hi $it — " } ?: "Hi — "
        parts.add(greeting + "you asked me to ring ${reasonPhrase(p.label)}.")
    ...
    fun instructions(
        ...
        return """
THIS IS A PHONE CALL the user asked you to make (call id ${p.callId}). Speak English, calm and brief, like a friend on the phone.
