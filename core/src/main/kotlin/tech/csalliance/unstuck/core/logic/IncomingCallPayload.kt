...
//   { kind:"call", callId, taskId?, title, notes?: JSON string[], scheduledAt: ISO,
//     deepLink:"unstuck://call/<callId>",
//     label, blockId?, taskName?, startTime?, firstAction?, estimateMin?: "N",
...
data class IncomingCallPayload(
    ...
    companion object {
         ...
         *  not a valid call push: `kind` present and not "call", or a blank
         *  `callId` / label. `label` wins over `title`; either satisfies the rule. */
        fun fromData(data: Map<String, String>): IncomingCallPayload? {
            val kind = data[KEY_KIND]?.trim()
            if (kind != null && kind.isNotEmpty() && kind != KIND) return null
