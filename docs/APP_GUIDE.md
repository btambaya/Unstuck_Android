...
Every number comes from one engine shared with the assistant's `get_period_review`: `:core/PeriodReview.kt` (the period resolver, the strict stamp grammar, `collectWindow` / `planFacts`) and `:core/PeriodFacts.kt` (the page's facts: headline, per-day rhythm, showed-up days, plan vs followed through, repeating-task dots, got-unstuck wins, done by area, the trend). Sessions always go through `:core/FocusFilter.kt` first (D1: under 60 s never counts; each session counts at most max(3× estimate, estimate + 60 min), capped at 4 h). `:core/Analytics.kt` keeps the pattern charts (area bars: your areas, then any other area a task carries under its own name, then No area; the 7×24 hour×day heatmap by the hours a session spanned, calibration, interruptions, pause anatomy, pause→resume "coming back", slip detection without repeating series or Later tasks). Cross-platform rules (2026-09-24, pinned in `CrossPlatformRulesTest`): repeating series order kept → due so far → name; a runaway session's start is completedAt − its real length (golden hours, interruptions); All time starts at the earliest task created / task done / counted session; get_insights minutes are floor((sec + 30) / 60).
