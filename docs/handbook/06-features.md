...
Above those, a task block gets the web modal's task actions (Ahmad 2026-09-24, "Can't complete a task from calendar"): **Start focus** (coral), **Mark done / Mark not done**, and **Open task**. `calBlockSheetActions` (core `CalBlockActions.kt`) resolves the block to the row Today shows — `taskForBlock`, so a repeating task's block is that day's OCCURRENCE (id = block id) — and the sheet hands that row to the existing paths: `vm.toggleDone` (ticks the day's block for an occurrence, never the series), the shell's `openFocus`, and `Route.Detail`. Google events, reserved time, shared-with-me blocks and a block whose task is gone get none of them; a task I assigned out gets only Open task. Tests: `CalBlockActionsTest` (core), `CalBlockSheetActionPathTest` + `CalBlockEditSheetTest` (app); `CalBlockEditSheetRenderTest` writes PNGs when `UNSTUCK_RENDER_DIR` is set.

The Week view opens the same sheet on a task block (as iOS and web do); it used to go straight to the task screen. The sheet's done, and the strike on the Day / Week grids and the Month peek, are one rule: `blockIsDone` (core `Occurrences.kt`, port of web `lib/occurrences` and iOS): a repeating task's day is done on its own block, a one-off when its task is, no task → not done. A shared-with-me block keeps the owner's `done`. The old `block.done || task.done` left a block struck after Mark not done when the day was ticked while the task repeated and the repeat was then set to Never (`taskAfterSettingRecurrence` carries the tick onto the task; the block keeps `done`).
