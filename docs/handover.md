...
- **Agentic assistant** — bubble = Assistant chat + Feedback; qwen via the stateless `assistant` edge fn + client-side tool execution; on-device $0 voice. **Superseded in v0.5.0** by the gateway port above (56 tools, memory, harness in `:core`).
