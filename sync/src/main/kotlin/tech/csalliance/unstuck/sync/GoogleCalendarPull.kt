 ...
 *
 * Pulls run one at a time, and [exclusive] lets a disconnect's local purge run between
 * them: an /events answer read before the revoke can then never land after the purge.
 * That is iOS's CalendarConnectionsReadGate in Android form — it also covers the block
 * writes, which the iOS gate leaves to the post-disconnect pull (C18f).
 */
internal class GoogleCalendarPull(
    ...
    /** Run [block] between pulls (never while one is reading or writing). */
    ...
    suspend fun pull(manual: Boolean = false): Boolean = mutex.withLock { pullLocked(manual) }

    private suspend fun pullLocked(manual: Boolean): Boolean {
        ...
        if (resp.failures.isNotEmpty()) {
            ...
            for (c in runCatching { listConnections() }.getOrNull().orEmpty()) {
                if (switched()) return true
